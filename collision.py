...
def collision_ball_paddle(ball, paddle, dt):

    """ Calculates the ball-paddle collision. Uses ball-block collision test
        to calculate if there is an impact.
    """

    # Get distance to impact
    distToImpact, side = collisionTest_ball_block( ball, paddle )
    ...
    # Getting positions of the paddle and the ball
    paddlePos = paddle.getPosition()
    ballPos = ball.getPosition()

    # Limiting the maximum bounce angle to 60 degrees as choice
    maxBounceAngle = math.pi / 3

    # This is the distance from middle of the paddle to the intersection of the ball
    relativeIntersectX = ballPos[0] - paddlePos[0]

    # By dividing the distance with half the width, we get a number between -1 and 1
    normalizedRelativeIntersectionX = (relativeIntersectX/(paddle.getWidth()/2))

    # Now we can calculate the bouncing angle as pi/2 + the angle depending
    # on where the ball hits (if it hits the corners, it will be 72 degrees)
    bounceAngle = (math.pi/2) - (normalizedRelativeIntersectionX*maxBounceAngle)

    # Ball's velocity in each direction is:
    # current velocity * paddle elasticity * ball elasticity * angle
    ballVx = vmag*paddle.getElasticity()*ball.getElasticity()*math.cos(bounceAngle)
    ballVy = vmag*paddle.getElasticity()*ball.getElasticity()*math.sin(bounceAngle)

    if side == 0 or side == 1: # left or right wall, so adjust x

        ball.setVelocity([-v[0]*ball.getElasticity()*paddle.getElasticity(), v[1]])
...
def collision_rotating_block_ball(rotating_block, ball, dt):
    return collision_ball_rotating_block(ball, block, dt)

def collision_paddle_ball(paddle, ball, dt):
    return collision_ball_paddle(ball, paddle, dt)
...
collision_router[ ('ball', 'rotating_block') ] = collision_ball_rotating_block
collision_router[ ('ball', 'paddle') ] = collision_ball_paddle
...
collision_router[ ('rotating_block', 'ball') ] = collision_rotating_block_ball
collision_router[ ('paddle', 'ball') ] = collision_paddle_ball
